    ...
    for (const char* className : hyperos_classes) {
        ...
        if (cls != nullptr) {
            ...
            if (isHardwareDetectedMethod != nullptr) {
                ...
                static const char* capturedClassName = className; // Simpan nama kelas di variabel lokal
                
                // Definisi function pointer untuk hook
                static jboolean (*hook_fp)() = nullptr;
                
                // Tetapkan function pointer ke fungsi yang mengembalikan true
                hook_fp = []() -> jboolean {
                    LOGI("[HookReplacement] isHardwareDetected dipanggil - mengembalikan TRUE");
                    return JNI_TRUE;
                };
                
                static JNINativeMethod methods[] = {
                    {"isHardwareDetected", "()Z", (void *)hook_fp}
                };
